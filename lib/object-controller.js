...
class ObjectController {
    ...
    setCacheDuration(durationMs) {
        ...
        this.log.debug(`Device Cache-Dauer auf ${durationMs}ms gesetzt`);
    }

    /**
     * Get devices with caching support
    ...
    async getDevicesWithCache() {
        ...
        if (cacheAge > this.cacheDuration || this.devices.length === 0) {
            this.log.debug('Cache abgelaufen oder leer, aktualisiere Geräteliste...');
            await this.discoverDevices(this.allowedRooms);
        } else {
            this.log.debug(`Verwende gecachte Geräteliste (${Math.round(cacheAge / 1000)}s alt)`);
            // Still refresh current values
            await this.refreshDeviceValues();
        }
    ...
    findDevice(identifier, room = null) {
        ...
        if (room && matches.length > 1) {
            ...
            const roomMatches = matches.filter(d =>
                d.room && typeof d.room === 'string' && d.room.toLowerCase().includes(roomLower)
            );
